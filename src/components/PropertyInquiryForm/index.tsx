...
const PropertyInquiryForm = ({
  ...
  const { loading } = useSelector(
    (state: any) => state.propertiesInquiries.new
  );
  ...
  const handleFormSubmit = async (data: any) => {
    ...
    if (formType === "update") {
      ...
      formData.append(
        "id_card_front",
        frontImage ? JSON.stringify(frontImage) : ""
      );
      formData.append(
        "id_card_back",
        backImage ? JSON.stringify(backImage) : ""
      );
  ...
  const handleImagesDeleted = (fieldName: string, deletedPaths: string[]) => {
    if (deletedPaths.length > 0) {
        ...
        const updatedFiles = prevData?.filter(
          (data: any) =>
            !(
              ...
              deletedPaths.includes(data?.image_urls)
            )
        );
      ...
      if (fieldName === "id_card_front") {
        setDeletedImagePathsFront((prev: string[]) => [
          ...prev,
          ...deletedPaths,
        ]);
      } else if (fieldName === "id_card_back") {
        setDeletedImagePathsBack((prev: string[]) => [
          ...prev,
          ...deletedPaths,
        ]);
      }
    ...
    <>
      <form onSubmit={handleSubmit(handleFormSubmit)}>
        <Box sx={{ mb: 5 }} className={`properties ${isMobile ? "sp" : ""}`}>
          <SectionTitle
            title={formType === "create" ? "顧客新規登録" : "顧客新規"}
          />
          <Box
          ...
          >
            <CustomTwoColInputGroup
              ...
              validationMessageLastName="名前は必須です"
              register={register}
              errors={errors}
              isRequired={true}
            ...
            <CustomTwoColInputGroup
              label="フリガナ"
              firstName="last_name_kana"
              lastName="first_name_kana"
              placeholderOne="ミョウジ"
              placeholderTwo="ナマエ"
              register={register}
              errors={errors}
              ...
              >
                <Box sx={{ marginTop: "18px" }}>
                  <FormImagesUploader
                    ...
                    register={register}
                    initialImages={
                      defaultValues?.customer?.id_card_back &&
                      defaultValues?.customer?.id_card_back !== ""
                        ? defaultValues?.customer?.id_card_back
                        : ""
                    }
                    type="single"
                    onImagesDeleted={(paths) =>
                      handleImagesDeleted("id_card_back", paths)
                    }
                  />
                </Box>
              </Box>
              ...
              >
                <Box sx={{ marginTop: "18px" }}>
                  <FormImagesUploader
                    ...
                    type="single"
                    onImagesDeleted={(paths) =>
                      handleImagesDeleted("id_card_front", paths)
                    }
                  />
                </Box>
              </Box>
        ...
        >
          <CustomButton
            ...
            type="submit"
            disabled={
              formType === "update" ? !isDirty : !isFormValid && !isDirty
            }
