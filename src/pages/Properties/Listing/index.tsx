...
import { deleteProperty, searchProperties } from "../../../store/propertiesSlice";
...
const PropertiesListing = () => {
  ...
  const fetchProperties = async (params:any, page: number = 1, limit: number = perPage) => {
    try {
      const searchPayload = {
      ...
      const cleanedPayload = Object.fromEntries(
        Object.entries(searchPayload).filter(([_, value]) => value !== undefined)
      );
  ...
  useEffect(() => {
    const sorted = [...propertyList];
      ...
      const rows = sorted.map((property: any, index: number) => ({
        ...
        inquiry_count: property.inquiry_count!==undefined&&property.inquiry_count!==''&&property.inquiry_count+'件' || "-",
        price: property.price || "-",
        type: property.type || "",
        land_area: property.details?.land_area!==undefined&&property.details?.land_area!==''&&property.details?.land_area+'㎡' || property.details?.private_area!==undefined&&property.details?.private_area!==''&&property.details?.private_area+'㎡' || "-",
  ...
  const handleDelete = async() => {
    setOpenDeleteModal(false);
        try {
            const deletedResult = await dispatch(deleteProperty(formattedDataToDelete));
  ...
  return (
    <Box py={{lg: 3, xs: 1}}>
      ...
      <Stack direction={"row"} justifyContent={"space-between"} mt={2.5}>
        <form onSubmit={handleSubmit(onSubmit)} style={{width: '100%'}}>
          ...
          <Box sx={{ display: { xs: 'block', md: 'none' } }}>
            ...
            <Stack direction={"column"} gap={{lg: 2, xs: 1}} marginBottom={2}>
              <Stack direction={"row"} justifyContent={{lg: "space-between", xs: 'start'}} gap={{lg: 2, xs: 1.5}}>
                <CustomSelect
                  label="担当者"
                  name="person1"
                  control={control}
                  options={[{value: "佐藤 次郎", label: "佐藤 次郎"}]}
                  defaultValue="佐藤 次郎"
                  sx={{ flex: 1}}
                />
                <CustomSelect
            ...
            <Stack direction={"row"} gap={1} marginBottom={3} alignItems={'center'}>
              <CustomTextField
                fullWidth
           ...
           <Box>
            <Stack 
              ...
              flexWrap={'wrap'}
            >
              <Typography sx={{ fontSize: {lg: '14px', xs: '10px'}, fontWeight: 700 }}>
                検索結果 <span style={{color: '#0B9DBD', margin: '0 7px'}}>{properties.length}</span> 件
              </Typography>
          ...
          <Box sx={{ display: { xs: 'none', md: 'block' } }}>
            <Stack direction={"row"} justifyContent={"flex-start"} gap={5} marginBottom={1}>
              <CustomSelect
                label="登録日時"
            ...
            <Stack direction={"row"} justifyContent={"flex-start"} gap={5} marginBottom={3}>
              <CustomSelect
                label="専有面積"
                name="exclusive_area"
                control={control}
            ...
            <Stack direction={"row"} justifyContent={"space-between"} flexWrap={'wrap'}>
              <Stack direction={"row"} justifyContent={"flex-end"} gap={1} marginBottom={3} alignItems={'center'}>
                <Typography marginRight={2} fontSize={12} fontWeight={700}>物件名</Typography>
                <CustomTextField
                  fullWidth
                  placeholder='キーワードで絞り込む'
                  {...register('objectName')}
              ...
              <Stack direction={"row"} justifyContent={"flex-end"} gap={1} marginBottom={3}>
                <CustomButton
                  label="新規物件登録"
                  type="button"
      ...
      <Box sx={{display: {xs: 'none', md: 'block'}}}>
        <Stack direction="row" spacing={1} marginBottom={2} alignItems={'center'} flexWrap={'wrap'}>
          <Typography sx={{ fontSize: '14px', fontWeight: 700, marginRight: '12px !important' }}>
              検索結果 <span style={{color: '#0B9DBD', margin: '0 7px'}}>{properties.length}</span> 件
          </Typography>
          <Typography fontSize={14} fontWeight={'700'} marginRight={'12px !important'}>⇅ 並び替え</Typography>
        ...
        <CustomModal title='削除の確認' openModal={openDeleteModal} handleCloseModal={()=>setOpenDeleteModal(false)} bodyText='物件を削除してよろしいですか？'>
